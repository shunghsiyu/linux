/*
 * CBMC Verification Harness for BPF verifier's scalar32_min_max_mul()
 *
 * Property being verified:
 *   For any valid src and dst register states containing values src_val and dst_val,
 *   after calling scalar32_min_max_mul(dst, src), the output dst register state's
 ...
 *   make -C tools/verification/cbmc/bpf scalar32_min_max_mul_verify
 *   make -C tools/verification/cbmc/bpf scalar32_min_max_mul_trace
 *   make -C tools/verification/cbmc/bpf scalar32_min_max_mul_simple
 */
...
static bool val32_in_reg(struct bpf_reg_state *reg, u64 val)
{
	...
	/* Note: We skip 64-bit and tnum checks because scalar32_min_max_mul()
 ...
 *
 * Verifies that scalar32_min_max_mul() correctly tracks the 32-bit
...
void main(void)
{
	...
	__CPROVER_assume(src_reg.s32_max_value <= 10);
	__CPROVER_assume(dst_reg.s32_min_value >= 0);
	__CPROVER_assume(dst_reg.s32_max_value <= 10);

	/* Use tiny ranges */
	__CPROVER_assume((src_reg.u32_max_value - src_reg.u32_min_value) <= 2);
	__CPROVER_assume((dst_reg.u32_max_value - dst_reg.u32_min_value) <= 2);
#endif

	/* ==================== Operation ==================== */

	/*
	 * Call scalar32_min_max_mul() to update dst_reg based on multiplying by src_reg.
	 ...
	 * For 32-bit multiplication, we care about the lower 32 bits.
	 */
	 ...
	 * (though we only check the parts that scalar32_min_max_mul touches)
	 */
