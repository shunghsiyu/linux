/*
 * CBMC Verification Harness for BPF verifier's scalar_min_max_mul()
 *
 * Property being verified:
 *   For any valid src and dst register states containing values src_val and dst_val,
 *   after calling scalar_min_max_mul(dst, src), the output dst register state must
 *   contain (dst_val * src_val), taking overflow into account.
 *
 * Note: Multiplication is complex because negative numbers can flip min/max relationships.
 *       The verifier computes all 4 corner products and takes min/max of those.
 *
 * To run this verification:
 *   make -C tools/verification/cbmc/bpf scalar_min_max_mul_verify
 *   make -C tools/verification/cbmc/bpf scalar_min_max_mul_trace
 *   make -C tools/verification/cbmc/bpf scalar_min_max_mul_simple
 */
...
static bool val_in_reg(struct bpf_reg_state *reg, u64 val)
{
	...
	/* Note: We skip 32-bit and tnum checks because scalar_min_max_mul()
 ...
 *
 * Verifies that scalar_min_max_mul() correctly tracks the 64-bit
 * signed and unsigned bounds after multiplication.
 */
void main(void)
{
	/* ==================== Setup ==================== */
	 ...
	 * Multiplication is very expensive for CBMC, so we use very small values.
	 */

	/* Use very small positive values */
	__CPROVER_assume(src_reg.smin_value >= 0);
	__CPROVER_assume(src_reg.smax_value <= 10);
	__CPROVER_assume(dst_reg.smin_value >= 0);
	__CPROVER_assume(dst_reg.smax_value <= 10);

	/* Use tiny ranges */
	__CPROVER_assume((src_reg.umax_value - src_reg.umin_value) <= 2);
	__CPROVER_assume((dst_reg.umax_value - dst_reg.umin_value) <= 2);
#endif

	/* ==================== Operation ==================== */

	/*
	 * Call scalar_min_max_mul() to update dst_reg based on multiplying by src_reg.
	 ...
	 * Note: In C, unsigned overflow wraps around (well-defined).
	 */
	 ...
	 * scalar_min_max_mul() updates. The full verifier would also
	 ...
	 * (though we only check the parts that scalar_min_max_mul touches)
	 */
