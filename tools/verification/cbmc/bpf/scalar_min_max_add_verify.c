/*
 * CBMC Verification Harness for BPF verifier's scalar_min_max_add()
 *
 * This file demonstrates verification of a binary operation (addition)
 * in the BPF verifier's abstract interpretation engine.
 *
 * Property being verified:
 *   For any valid src and dst register states containing values src_val and dst_val,
 *   after calling scalar_min_max_add(dst, src), the output dst register state must
 *   contain (dst_val + src_val), taking overflow into account.
 *
 * To run this verification:
 *   make -C tools/verification/cbmc/bpf scalar_min_max_add_verify
 *   make -C tools/verification/cbmc/bpf scalar_min_max_add_trace
 *   make -C tools/verification/cbmc/bpf scalar_min_max_add_simple
 */
...
static bool val_in_reg(struct bpf_reg_state *reg, u64 val)
{
	...
	/* Note: We skip 32-bit and tnum checks because scalar_min_max_add()
 ...
 *
 * Verifies that scalar_min_max_add() correctly tracks the 64-bit
...
void main(void)
{
	...
	/*
	 * Call scalar_min_max_add() to update dst_reg based on adding src_reg.
	 ...
	 * scalar_min_max_add() updates. The full verifier would also
	 ...
	 * (though we only check the parts that scalar_min_max_add touches)
	 */
