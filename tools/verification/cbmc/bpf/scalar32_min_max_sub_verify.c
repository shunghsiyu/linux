/*
 * CBMC Verification Harness for BPF verifier's scalar32_min_max_sub()
 *
 * Property being verified:
 *   For any valid src and dst register states containing values src_val and dst_val,
 *   after calling scalar32_min_max_sub(dst, src), the output dst register state's
 *   32-bit bounds must contain the lower 32 bits of (dst_val - src_val).
 *
 * To run this verification:
 *   make -C tools/verification/cbmc/bpf scalar32_min_max_sub_verify
 *   make -C tools/verification/cbmc/bpf scalar32_min_max_sub_trace
 *   make -C tools/verification/cbmc/bpf scalar32_min_max_sub_simple
 */
...
static bool val32_in_reg(struct bpf_reg_state *reg, u64 val)
{
	...
	/* Note: We skip 64-bit and tnum checks because scalar32_min_max_sub()
 ...
 *
 * Verifies that scalar32_min_max_sub() correctly tracks the 32-bit
...
void main(void)
{
	...
	/*
	 * Call scalar32_min_max_sub() to update dst_reg based on subtracting src_reg.
	 ...
	 * For 32-bit subtraction, we care about the lower 32 bits.
	 */
	u32 dst_val32 = (u32)dst_val;
	u32 src_val32 = (u32)src_val;
	u32 result_val32 = dst_val32 - src_val32;  /* 32-bit underflow wraps */
	 ...
	 * (though we only check the parts that scalar32_min_max_sub touches)
	 */
