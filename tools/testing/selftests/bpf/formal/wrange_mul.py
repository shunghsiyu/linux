...
def main():
    ...
    print('\nChecking {0xff0..0xfff} * {0xf0..0xff} = {0xef100..0xfef01}')
    ...
    # Use the 8-bit version otherwise it will take too long
    BitVec8 = lambda n: BitVec(n, bv=8)
    BitVecVal8 = lambda v: BitVecVal(v, bv=8)

    class Wrange8(Wrange):
        SIZE = 8
    ...
    # Suppose we have a wrange8 called w1 that contains the 8-bit integer x
    # (where x can be any possible value contained inside w1), and another
    # wrange8 called w2 that similarly contains the 8-bit integer y.
    #
    # The product of w1 and w2 calculated from wrange_mul(w1, w2), called w,
    # should _always_ contains the product of x and y, no matter what.
    print('\nChecking that if w1.contains(x) and w2.contains(y),\nthen (the 8-bit version of) wrange32_mul(w1, w2).contains(x*y)')
